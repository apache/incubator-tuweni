...
public class Blake2bfMessageDigest extends BCMessageDigest implements Cloneable {
  ...
  public static class Blake2bfDigest implements Digest {
    ...
    public void update(final byte in) {
      ...
      if (buffer.size() == MESSAGE_LENGTH_BYTES) { // full buffer
        throw new IllegalArgumentException();
      } else {
        buffer = Bytes.wrap(buffer, Bytes.of(in));
      }
    }
    ...
    public void update(final byte[] in, final int offset, final int len) {
      Bytes value = Bytes.wrap(in, offset, len);
      if (value.size() > MESSAGE_LENGTH_BYTES - buffer.size()) {
        throw new IllegalArgumentException(
            "Attempting to update buffer with "
                + value.size()
                + " byte(s) but there is "
                + (MESSAGE_LENGTH_BYTES - value.size())
                + " byte(s) left to fill");
      }
    ...
    private void compress(byte[] out) {
      ...
      for (int i = 0; i < h.length; i++) {
        ...
        h[i] = buffer.getLong(offset, ByteOrder.LITTLE_ENDIAN);
      }

      for (int i = 0; i < 16; i++) {
        final int offset = 68 + i * 8;
        m[i] = buffer.getLong(offset, ByteOrder.LITTLE_ENDIAN);
      }

      t[0] = buffer.getLong(196, ByteOrder.LITTLE_ENDIAN);
      t[1] = buffer.getLong(204, ByteOrder.LITTLE_ENDIAN);

      boolean f = buffer.get(212) != 0;
