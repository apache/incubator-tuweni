...
class WhoAreYouMessageTest {
  ...
  fun encodeAndDecodeCorrectlyTransformsData() {
    ...
    val idNonce = UdpMessage.idNonce()

    val message = WhoAreYouMessage(srcId, destId, authTag, idNonce)

    val encodedResult = message.encode()

    val content = Bytes.wrapByteBuffer(encodedResult).slice(UdpMessage.TAG_LENGTH)
    val decodedMessage = WhoAreYouMessage.create(content, srcId, destId)

    assert(decodedMessage.authTag == authTag)
    assert(decodedMessage.idNonce == idNonce)
    assert(decodedMessage.dest == destId)
  }
