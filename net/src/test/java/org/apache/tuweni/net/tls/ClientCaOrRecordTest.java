...
class ClientCaOrRecordTest {
  ...
  void shouldValidateUsingCertificate() throws Exception {
    ...
    assertEquals(2, knownServers.size(), "CA verified host should not have been recorded");
  ...
  void shouldFallbackToRecordingForInvalidName() throws Exception {
    ...
    assertEquals(
        "localhost:" + foobarServer.actualPort() + " " + DUMMY_FINGERPRINT, knownServers.get(1));
    assertEquals(
        "127.0.0.1:" + caValidServer.actualPort() + " " + caValidFingerprint, knownServers.get(2));
  }
